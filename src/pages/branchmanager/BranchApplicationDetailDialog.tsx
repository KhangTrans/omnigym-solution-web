...
export function BranchApplicationDetailDialog({ application, open, onOpenChange, processing, approvedLevel, setApprovedLevel, onApprove, onRejectClick }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto border-0 shadow-lg admin-scrollbar">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            Hồ sơ Trainer #{application.id}
            <StatusBadge status={application.status} />
          </DialogTitle>
          <DialogDescription>Kiểm tra hồ sơ, giấy tờ định danh và chứng chỉ trước khi duyệt.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          <section className="grid gap-4 md:grid-cols-[160px_1fr]">
            <div className="overflow-hidden rounded-xl bg-muted shadow-sm">
              {application.avatar_url ? (
                <img src={application.avatar_url} alt="Avatar" className="h-40 w-full object-cover" />
              ) : (
                <div className="grid h-40 place-items-center text-muted-foreground"><User className="h-8 w-8" /></div>
              )}
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <InfoItem icon={<User className="h-3.5 w-3.5" />} label="Người gửi" value={applicantName(application)} />
          ...
          <section>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <InfoItem icon={<ShieldCheck className="h-3.5 w-3.5" />} label="Số định danh" value={application.identity_number} />
              <div className="rounded-lg bg-muted/40 p-3 shadow-sm">
                <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Ảnh định danh</div>
                {application.identity_image_url ? (
                  <a href={application.identity_image_url} target="_blank" rel="noreferrer" className="block overflow-hidden rounded-lg bg-background shadow-sm">
                    <img src={application.identity_image_url} alt="Identity" className="max-h-56 w-full object-contain bg-background" />
                  </a>
          ...
          <section>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {application.certificates?.length ? application.certificates.map((cert, index) => (
                <div key={cert.id || index} className="rounded-xl bg-card p-4 shadow-sm">
                  <div className="mb-3 flex items-center gap-2 font-semibold"><FileBadge className="h-4 w-4" />{cert.cert_name || `Certificate #${index + 1}`}</div>
                  <div className="grid gap-2 text-sm">
                    <InfoItem label="Đơn vị cấp" value={cert.issued_by} />
                    <InfoItem label="Mã chứng chỉ" value={cert.certificate_number} />
                    <InfoItem label="Ngày cấp" value={cert.issued_at ? String(cert.issued_at).slice(0, 10) : "-"} />
                    <InfoItem label="Ngày hết hạn" value={cert.expires_at ? String(cert.expires_at).slice(0, 10) : "-"} />
                  </div>
                  {cert.image_url && <a href={cert.image_url} target="_blank" rel="noreferrer" className="mt-3 block overflow-hidden rounded-lg bg-background shadow-sm"><img src={cert.image_url} alt={cert.cert_name || "Certificate"} className="max-h-52 w-full object-contain bg-background" /></a>}
                </div>
              )) : <div className="rounded-lg bg-muted/40 p-4 text-sm text-muted-foreground shadow-sm">Không có chứng chỉ.</div>}
            </div>
          </section>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          {application.status === "pending" && <>
            <Button variant="destructive" onClick={onRejectClick} disabled={processing}><XCircle className="h-4 w-4" />Từ chối</Button>
            <Button onClick={() => onApprove(application.id)} disabled={processing} className="bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground">{processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Duyệt hồ sơ</Button>
          </>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
