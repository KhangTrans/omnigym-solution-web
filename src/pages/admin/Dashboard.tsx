...
function AdminOverview() {
  ...
  const cards = [
    { label: "Tổng doanh thu", value: fmt(stats.total), icon: DollarSign, hint: `${fmt(stats.last30)} 30 ngày qua` },
    { label: "Hội viên hoạt động", value: String(stats.activeMembers), icon: Users, hint: `${users.length} tổng tài khoản` },
    { label: "Gói bài tập", value: String(packs.filter((p) => p.publishedToDashboard).length), icon: Dumbbell, hint: `${packs.length} tổng · đang hiển thị` },
    { label: "Bài tập thư viện", value: String(library.length), icon: Library, hint: "Available to add to packs" },
  ];
          ...
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <Button asChild size="sm" variant="ghost">
              <Link to="/admin/revenue">
                Mở <ArrowRight className="ml-1 h-3 w-3" />
              </Link>
            </Button>
          </CardHeader>
          <CardContent>
